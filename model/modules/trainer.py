from model.modules.optim import ScheduledOptim, LabelSmoothingLoss
from torch.optim import Adam, SGD, AdamW
from torch import nn
from model.modules.tools import _build_model, _translate
from model.modules.data import (
    ImgAugTransform,
    DataGen,
...
from model.utils import Logger, download_weights
from model.modules.tools import compute_accuracy
...
class Trainer():
    def __init__(self, config, model=None, pretrained=True, augmentor=ImgAugTransform()):

        self.config = config
        if model:
            _, self.vocab = _build_model(config)
            self.model = model
        else:
            self.model, self.vocab = _build_model(config)
    ...
    def predict(self, sample=None):
        ...
        for batch in  self.valid_gen:
            ...
            translated_sentence, prob = _translate(batch['img'], self.model)
